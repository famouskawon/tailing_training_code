...
def main(flags):
    ...
    if flags.is_master:
        model_summary(model, (16, 3, 224, 224))
            ...
            if best_val_acc < test_acc and epoch > 0:
                ...
                if flags.save_dir is not None:
                    torch.save({
                        ...
                        'acc' : best_val_acc,
                    }, os.path.join(flags.save_dir, f'ViViT_UBI_best_val.ckp'))
            ...
            if train_loss < best_loss and epoch > 0:
                ...
                if flags.save_dir is not None:
                    torch.save({
                        'epoch': epoch,
                        'model': model.state_dict(),
                    }, os.path.join(flags.save_dir, f'ViViT_UBI_best_loss.ckp'))
...
def evaluation(epoch, model, dataloader, criterion, device, flags):
    ...
    if flags.is_master:
        ...
        print(f'{classification_report(y_true_total, y_pred_total, target_names=["fight", "normal"], digits=4)}')
...
if __name__ == '__main__':
    ...
    wandb.init(project = 'Tailing project', entity='chowk', name = "ViViT UBI_rgb_224B", reinit=True)
